...
bool CheckBlockSignature(const CBlock& block)
{
    ...
    /** Each block is signed by the private key of the input that is staked. This can be either zMAT or normal UTXO
     *  zMAT: Each zMAT has a keypair associated with it. The serial number is a hash of the public key.
     *  UTXO: The public key that signs must match the public key associated with the first utxo of the coinstake tx.
     */
    CPubKey pubkey;
    bool fzMATStake = block.vtx[1]->vin[0].IsZerocoinSpend();
    if (fzMATStake) {
        libzerocoin::CoinSpend spend = ZMATModule::TxInToZerocoinSpend(block.vtx[1]->vin[0]);
