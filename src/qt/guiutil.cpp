...
// Copyright (c) 2015-2021 The MATX Core developers
...
QString formatBalance(CAmount amount, int nDisplayUnit, bool isZmat)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZmat)) : BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZmat);
}

QString formatBalanceWithoutHtml(CAmount amount, int nDisplayUnit, bool isZmat)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZmat)) : BitcoinUnits::floorWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZmat);
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter MATX address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no MATX: URI
...
{
    // Convert matx:// to matx:
    //
    //    Cannot handle this later, because matx:// will cause Qt to see the part after // as host,
...
bool openConfigfile()
{
    return openFile(GetConfigFile(gArgs.GetArg("-conf", MATX_CONF_FILENAME)), true);
}
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "MATX (testnet).lnk";
    else if (chain == CBaseChainParams::REGTEST)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "MATX (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "MATX.lnk";
}
...
{
    return GetAutostartDir() / "matx.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a matx.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=MATX (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=MATX (regtest)\n";
        else
            optionFile << "Name=MATX\n";
