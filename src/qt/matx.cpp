...
#include "qt/matx/splash.h"
#include "qt/matx/welcomecontentwidget.h"
...
#ifdef ENABLE_WALLET
#include "qt/matx/governancemodel.h"
...
{
    return QCoreApplication::translate("matx-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in matx.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in matx.qrc)
...
/** Class encapsulating MATX Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (MATXGUI)
...
{
    window = new MATXGUI(networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &MATXGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &MATXGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(MATXGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(MATXGUI::DEFAULT_WALLET);
        ...
        // MATX: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &MATXGUI::handlePaymentRequest);
        connect(window, &MATXGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. MATX can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(matx_locale);
    Q_INIT_RESOURCE(matx);
    ...
    /// 6. Determine availability of data directory and parse matx.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", MATX_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // matx: links repeatedly have their payment requests routed to this process:
