    ...
    for (unsigned int nOut = 0; nOut < wtx.tx->vout.size(); nOut++) {
        ...
        if (ExtractDestination(txout.scriptPubKey, address)) {
            ...
            if (sub.address.empty() && txout.scriptPubKey.StartsWithOpcode(OP_RETURN)) {
                ...
                // Burned MATs, op_return could be for a kind of data stored there. For now, support UTF8 comments.
...
void TransactionRecord::updateStatus(const CWalletTx& wtx, int chainHeight)
{
    ...
    // For generated transactions, determine maturity
            ...
            type == TransactionRecord::StakeZMAT ||
...
{
    return type == TransactionRecord::StakeMint || type == TransactionRecord::Generated || type == TransactionRecord::StakeZMAT;
}
