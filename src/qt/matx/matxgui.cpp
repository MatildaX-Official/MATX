// Copyright (c) 2019-2022 The MATX Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "qt/matx/matxgui.h"
...
#include "qt/matx/qtutils.h"
#include "qt/matx/defaultdialog.h"
#include "shutdown.h"
...
const QString MATXGUI::DEFAULT_WALLET = "~Default";

MATXGUI::MATXGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void MATXGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    connect(toggleHideAction, &QAction::triggered, this, &MATXGUI::toggleHidden);
...
void MATXGUI::connectActions()
{
    ...
    connect(topBar, &TopBar::showHide, this, &MATXGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &MATXGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &MATXGUI::showHide);
    connect(sendWidget, &SendWidget::showHide, this, &MATXGUI::showHide);
    connect(receiveWidget, &ReceiveWidget::showHide, this, &MATXGUI::showHide);
    connect(addressesWidget, &AddressesWidget::showHide, this, &MATXGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::showHide, this, &MATXGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::execDialog, this, &MATXGUI::execDialog);
    connect(coldStakingWidget, &ColdStakingWidget::showHide, this, &MATXGUI::showHide);
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &MATXGUI::execDialog);
    connect(governancewidget, &GovernanceWidget::showHide, this, &MATXGUI::showHide);
    connect(governancewidget, &GovernanceWidget::execDialog, this, &MATXGUI::execDialog);
    connect(settingsWidget, &SettingsWidget::execDialog, this, &MATXGUI::execDialog);
}


void MATXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
...
void MATXGUI::handleRestart(QStringList args)
{
...
void MATXGUI::setClientModel(ClientModel* _clientModel)
{
    ...
    if (this->clientModel) {
        ...
        connect(clientModel, &ClientModel::message, this, &MATXGUI::message);
...
void MATXGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &MATXGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler* dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &MATXGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void MATXGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void MATXGUI::macosDockIconActivated()
 {
     show();
     activateWindow();
 }
...
void MATXGUI::changeEvent(QEvent* e)
{
    ...
    if (e->type() == QEvent::WindowStateChange) {
        if (clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray()) {
            ...
            if (!(wsevt->oldState() & Qt::WindowMinimized) && isMinimized()) {
                QTimer::singleShot(0, this, &MATXGUI::hide);
                e->ignore();
            } else if ((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized()) {
                QTimer::singleShot(0, this, &MATXGUI::show);
                e->ignore();
            }
        }
    }
...
void MATXGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void MATXGUI::messageInfo(const QString& text)
{
...
void MATXGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
...
bool MATXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
...
void MATXGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void MATXGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MATXGUI::detectShutdown()
{
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
...
void MATXGUI::goToSend()
{
    showTop(sendWidget);
}

void MATXGUI::goToAddresses()
{
    showTop(addressesWidget);
}

void MATXGUI::goToMasterNodes()
{
    masterNodesWidget->resetCoinControl();
    showTop(masterNodesWidget);
}

void MATXGUI::goToColdStaking()
{
    showTop(coldStakingWidget);
}

void MATXGUI::goToGovernance()
{
    showTop(governancewidget);
}

void MATXGUI::goToSettings(){
    showTop(settingsWidget);
}

void MATXGUI::goToSettingsInfo()
{
    navMenu->selectSettings();
...
void MATXGUI::goToReceive()
{
    showTop(receiveWidget);
}

void MATXGUI::openNetworkMonitor()
{
    settingsWidget->openNetworkMonitor();
}

void MATXGUI::showTop(QWidget* view)
{
...
void MATXGUI::changeTheme(bool isLightTheme)
{
...
void MATXGUI::resizeEvent(QResizeEvent* event)
{
    // Parent..
...
bool MATXGUI::execDialog(QDialog *dialog, int xDiv, int yDiv)
{
    return openDialogWithOpaqueBackgroundY(dialog, this);
}

void MATXGUI::showHide(bool show)
{
...
int MATXGUI::getNavWidth()
{
    return this->navMenu->width();
}

void MATXGUI::openFAQ(SettingsFaqWidget::Section section)
{
    showHide(true);
...
#ifdef ENABLE_WALLET
void MATXGUI::setGovModel(GovernanceModel* govModel)
{
    if (!stackedContainer || !clientModel) return;
    governancewidget->setGovModel(govModel);
}

void MATXGUI::setMNModel(MNModel* _mnModel)
{
    if (!stackedContainer || !clientModel) return;
    mnModel = _mnModel;
...
bool MATXGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    // Connect actions..
    connect(walletModel, &WalletModel::message, this, &MATXGUI::message);
    connect(masterNodesWidget, &MasterNodesWidget::message, this, &MATXGUI::message);
    connect(coldStakingWidget, &ColdStakingWidget::message, this, &MATXGUI::message);
    connect(topBar, &TopBar::message, this, &MATXGUI::message);
    connect(sendWidget, &SendWidget::message,this, &MATXGUI::message);
    connect(receiveWidget, &ReceiveWidget::message,this, &MATXGUI::message);
    connect(addressesWidget, &AddressesWidget::message,this, &MATXGUI::message);
    connect(governancewidget, &GovernanceWidget::message,this, &MATXGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &MATXGUI::message);

    // Pass through transaction notifications
    connect(dashboard, &DashboardWidget::incomingTransaction, this, &MATXGUI::incomingTransaction);

    return true;
}

bool MATXGUI::setCurrentWallet(const QString& name)
{
    // Single wallet supported.
    return true;
}

void MATXGUI::removeAllWallets()
{
    // Single wallet supported.
}

void MATXGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
...
static bool ThreadSafeMessageBox(MATXGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void MATXGUI::subscribeToCoreSignals()
{
...
void MATXGUI::unsubscribeFromCoreSignals()
{
