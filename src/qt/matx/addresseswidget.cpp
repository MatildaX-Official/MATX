...
#include "qt/matx/forms/ui_addresseswidget.h"
#include "qt/matx/addressfilterproxymodel.h"
#include "qt/matx/addresslabelrow.h"
#include "qt/matx/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(MATXGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto matAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStaking, isExchange, isShield);

        if (!Standard::IsValidDestination(matAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(matAdd);
        if (walletModel->updateAddressBookLabels(matAdd, label.toUtf8().constData(),
