...
#include "qt/matx/forms/ui_snackbar.h"
#include "qt/matx/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(MATXGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &MATXGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
