...
#include "qt/matx/dashboardwidget.h"
#include "qt/matx/forms/ui_dashboardwidget.h"
#include "qt/matx/sendconfirmdialog.h"
#include "qt/matx/txrow.h"
...
DashboardWidget::DashboardWidget(MATXGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareMat, "square-chart-mat");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelMat, "text-chart-mat");
    ...
    setCssProperty(ui->labelAmountMat, "text-stake-mat-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &MATXGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isMat = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair MAT, MN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            mat = (pair.first != 0) ? pair.first / 100000000 : 0;
            mn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalMat += pair.first;
            chartData->totalMN += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesMat.append(mat);
        chartData->valuesMN.append(mn);

        int max = std::max(mat, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesMat);
    set1->append(chartData->valuesMN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalMat > 0 || chartData->totalMN > 0) {
        setCssProperty(ui->labelAmountMat, "text-stake-mat");
        setCssProperty(ui->labelAmountMN, "text-stake-mn");
    } else {
        setCssProperty(ui->labelAmountMat, "text-stake-mat-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-mn-disable");
    }
    forceUpdateStyle({ui->labelAmountMat, ui->labelAmountMN});
    ui->labelAmountMat->setText(GUIUtil::formatBalance(chartData->totalMat, nDisplayUnit));
    ...
    if (fShowCharts) {
        if (!stakesFilter) {
            ...
            stakesFilter->setTypeFilter(TransactionFilterProxy::TYPE(TransactionRecord::StakeMint) |
                                        ...
                                        TransactionFilterProxy::TYPE(TransactionRecord::StakeZMAT) |
