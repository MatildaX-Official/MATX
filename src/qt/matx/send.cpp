...
#include "qt/matx/addnewcontactdialog.h"
#include "qt/matx/forms/ui_send.h"
#include "qt/matx/guitransactionsutils.h"
#include "qt/matx/loadingdialog.h"
#include "qt/matx/optionbutton.h"
#include "qt/matx/qtutils.h"
#include "qt/matx/sendchangeaddressdialog.h"
#include "qt/matx/sendconfirmdialog.h"
...
SendWidget::SendWidget(MATXGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a MATX URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-mat");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onMATSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onMATSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent MATs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield MATs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent MATs to shield."));
    }
}
...
void SendWidget::onMATSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto matAdd = Standard::DecodeDestination(address.toStdString(), isStaking, isExchange, isShielded);

        if (!Standard::IsValidDestination(matAdd) || isStaking) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(matAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(matAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
