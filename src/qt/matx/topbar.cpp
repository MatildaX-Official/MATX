...
#include "qt/matx/topbar.h"
#include "qt/matx/forms/ui_topbar.h"
#include "qt/matx/lockunlock.h"
#include "qt/matx/qtutils.h"
#include "qt/matx/receivedialog.h"
#include "qt/matx/loadingdialog.h"
#include "askpassphrasedialog.h"

#include "bitcoinunits.h"
#include "qt/matx/balancebubble.h"
...
TopBar::TopBar(MATXGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopMat, ui->labelAmountTopShieldedMat}, "amount-small-topbar");
    setCssProperty({ui->labelAmountMat}, "amount-topbar");
    setCssProperty({ui->labelPendingMat, ui->labelImmatureMat}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("MATX")
    updateDisplayUnit();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // MAT Total
    QString totalMat = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);
    ...
    // MAT
    // Top
    ui->labelAmountTopMat->setText(totalTransparent);
    ui->labelAmountTopShieldedMat->setText(totalShielded);
    // Expanded
    ui->labelAmountMat->setText(totalMat);
    ui->labelPendingMat->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance + newBalance.unconfirmed_shielded_balance, nDisplayUnit));
    ui->labelImmatureMat->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
