...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select MAT Outputs to Spend" :
            "Select Shielded MAT to Spend");
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show MATX address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
