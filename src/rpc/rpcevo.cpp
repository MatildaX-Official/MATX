...
// Copyright (c) 2021-2022 The MATX Core developers
...
static const std::map<ProRegParam, std::string> mapParamHelp = {
        {collateralAddress,
            "%d. \"collateralAddress\"     (string, required) The MATX address to send the collateral to.\n"
        },
        {collateralHash,
        ...
        {ownerAddress,
            "%d. \"ownerAddress\"          (string, required) The MATX address to use for payee updates and proposal voting.\n"
        ...
        {payoutAddress_register,
            "%d. \"payoutAddress\"          (string, required) The MATX address to use for masternode reward payments.\n"
        },
        {payoutAddress_update,
            "%d. \"payoutAddress\"          (string, required) The MATX address to use for masternode reward payments.\n"
...
// Allows to specify MATX address or priv key (as strings). In case of MATX address, the priv key is taken from the wallet
...
static CKeyID ParsePubKeyIDFromAddress(const std::string& strAddress)
{
    ...
    if (!keyID) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprintf("invalid MATX address %s", strAddress));
    }
    return *keyID;
}
