...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("MATXMiner : generated block is stale");
    }
    ...
    if (!res || sc.get().stateErrorFound()) {
        return error("MATXMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("MATXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("matx-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running MATXMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("MATXMiner exception");
    } catch (...) {
        LogPrintf("MATXMiner exception");
    }

    LogPrintf("MATXMiner exiting\n");
}
