...
#if defined(NDEBUG)
#error "MATX cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in umat) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k MAT
...
{
    util::ThreadRename("matx-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC_V2) &&
        ...
        if (pindex->nHeight % 50000 == 0) {
            ZMATModule::CleanCoinSpendsCache();
        }
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Matx Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nMatxBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nMatxBadBlockBits)) {
            // accept MATX block minted with incorrect proof of work threshold
            return true;
        }
                                     ...
                                     std::set<CBigNum>& spent_serials)
{
    ...
    const bool zmatActive = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC);
    const bool publicZmatActive = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_PUBLIC);
    ...
    for (size_t i = 1; i < block.vtx.size(); i++) {
        ...
        for (const CTxIn& in: tx->vin) {
            ...
            if (isPublicSpend && (!publicZmatActive || v5Active)) {
            ...
            if (isPrivZerocoinSpend && (!zmatActive || publicZmatActive)) {
            ...
            if (isPrivZerocoinSpend || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZMATModule::ParseZerocoinPublicSpend(in, *tx, state, publicSpend)){
                        return false;
                    }
                    spend = publicSpend;
                } else {
                    spend = ZMATModule::TxInToZerocoinSpend(in);
                }
        ...
        for (auto it = bl.vtx.rbegin(); it != bl.vtx.rend(); ++it) {
            ...
            for (const CTxIn& in: tx->vin) {
                ...
                if (!in.IsZerocoinSpend()) {
                    ...
                    // zerocoin serial
                    const CBigNum& s = ZMATModule::TxInToZerocoinSpend(in).getCoinSerialNumber();
...
static bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, const FlatFilePos* dbp) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (isPoS) {
        ...
        if (coinstake_in.IsZerocoinSpend()) {
            libzerocoin::CoinSpend spend = ZMATModule::TxInToZerocoinSpend(coinstake_in);
            if (!ContextualCheckZerocoinSpend(coinstake, &spend, pindex->nHeight)) {
                return state.DoS(100,error("%s: main chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                        coinstake.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zmat");
            }
        }
