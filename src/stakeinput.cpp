// Copyright (c) 2017-2022 The MATX Core developers
...
CMatStake* CMatStake::NewMatStake(const CTxIn& txin, int nHeight, uint32_t nTime)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CMatStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    if (!coin.IsSpent()) {
        ...
        // All good
        return new CMatStake(coin.out, txin.prevout, pindexFrom);
    }
    ...
    // All good
    return new CMatStake(txPrev->vout[txin.prevout.n], txin.prevout, pindexFrom);
}

bool CMatStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

CTxIn CMatStake::GetTxIn() const
{
    return CTxIn(outpointFrom.hash, outpointFrom.n);
}

CAmount CMatStake::GetValue() const
{
    return outputFrom.nValue;
}

CDataStream CMatStake::GetUniqueness() const
{
    //The unique identifier for a MAT stake is the outpoint
...
const CBlockIndex* CMatStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CMatStake: uninitialized pindexFrom");
    return pindexFrom;
}
