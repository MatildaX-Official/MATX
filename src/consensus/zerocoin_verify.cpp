...
static bool CheckZerocoinSpend(const CTransactionRef _tx, CValidationState& state)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZMATModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
                return state.DoS(100, error("%s: public zerocoin spend parse failed", __func__));
            }
            newSpend = publicSpend;
        } else {
            newSpend = ZMATModule::TxInToZerocoinSpend(txin);
        }
        ...
        if (isPublicSpend) {
            ...
            if (!ZMATModule::validateInput(txin, prevOut, tx, ret)){
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const libzerocoin::CoinSpend* spend, int nHeight)
{
    ...
    if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
        return error("%s : zMAT spend with serial %s is already in block %d\n", __func__,
...
{
    const Consensus::Params& consensus = Params().GetConsensus();
    //Check to see if the zMAT is properly signed
    if (consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2)) {
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 zMAT spend does not have a valid signature\n", __func__);
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend zMAT without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend->HasValidSerial(consensus.Zerocoin_Params(fUseV1Params)))  {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(nHeight))
            return error("%s : zMAT spend with serial %s from tx %s is not in valid range\n", __func__,
                                    ...
                                    std::vector<std::pair<CBigNum, uint256>>& vSpendsRet)
{
    for (const CTxIn& txIn : tx.vin) {
        ...
        if (isPublicSpend) {
            libzerocoin::ZerocoinParams* params = consensus.Zerocoin_Params(false);
            PublicCoinSpend publicSpend(params);
            if (!ZMATModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)) {
                return false;
            }
