 ...
 */
const char * const MATX_PID_FILENAME = "matx.pid";

fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", MATX_PID_FILENAME));
...
void Shutdown()
{
    ...
    util::ThreadRename("matx-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf("Specify configuration file (default: %s)", MATX_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", MATX_PID_FILENAME));
#endif
...
{
    util::ThreadRename("matx-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that MATX is running in a usable environment with all
...
{
    // Make sure only a single MATX process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", MATX_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile because if MATX is started in the future "
                  "from a different location. It will be unable to locate the current data files. There could "
                  "also be data loss if MATX is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //MATX specific: zerocoin and spork DB's
                ...
                // MATX: load previous sessions sporks if we have them.
