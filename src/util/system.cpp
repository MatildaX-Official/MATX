...
const char * const MATX_CONF_FILENAME = "matx.conf";
const char * const MATX_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "matxd -foo=bar
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\MATX
// Windows >= Vista: C:\Users\Username\AppData\Roaming\MATX
// Mac: ~/Library/Application Support/MATX
// Unix: ~/.matx
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MATX";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MATXParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MATXParams
    // Mac: ~/Library/Application Support/MATXParams
    // Unix: ~/.matx-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MATXParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == nullptr || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "MATXParams";
#else
    // Unix
    return pathRet / ".matx-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/matx/sapling-spend.params";
        sapling_output = "/usr/share/matx/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/matx/sapling-spend.params";
            sapling_output = "/usr/local/share/matx/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", MATX_MASTERNODE_CONF_FILENAME));
