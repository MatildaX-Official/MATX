...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/MATXParams"
    else
        PARAMS_DIR="$HOME/.matx-params"
    fi
fi
...
function main() {
    ...
    cat <<EOF
MATX - install-params.sh

This script will install the MATX zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common MATX zkSNARK parameters. Note that it is
