...
*Setup instructions for a gitian build of MATX Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the MATX
...
to the MATX GitHub release page.
...
- [Building MATX Core](#building-matx-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/matx-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/matx-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/matx-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/matx-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/matx-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/matx-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/matx-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@matx.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/matx-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/matx-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/matx-osx-*-build.assert
```
