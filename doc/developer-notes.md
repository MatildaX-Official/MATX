...
- [Development guidelines](#development-guidelines)
    - [General MATX Core](#general-matx-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
MATX Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play MATs (tMAT)" on the test network, if you
...
MATX Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/matx-project/matx/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_matx
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_matx --log_level=test_suite
$ valgrind -v --leak-check=full src/matxd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_matx.coverage/index.html`.
```

**Sanitizers**
MATX can be compiled with various "sanitizers" enabled, which add
...
- BitcoinMiner : Generates MATs (if wallet is enabled).
...
However, in open source software such as MATX Core, where everyone uses
...
pay attention to for reviewers of MATX Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: MATX Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against MATX Core so that they can be integrated
...
- src/chiabls
  - Upstream at https://github.com/MatildaX-Official/bls-signatures/ ; actively maintained by Core contributors, deviates from upstream https://github.com/Chia-Network/bls-signatures/
...
file descriptors it will cause problems with MATX's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:MatildaX-Official/MATX.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `matx-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `matx-cli` and the GUI debug console use this table to determine how to
