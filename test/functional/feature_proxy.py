...
"""Test matxd with different proxy configuration.

Test plan:
- Start matxd's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on matxd side:
...
- Create matxds that connect to them
- Manipulate the matxds using addnode (onetry) an observe effects
...
class ProxyTest(MatxTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: matxd's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
