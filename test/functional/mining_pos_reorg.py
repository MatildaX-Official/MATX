...
class ReorgStakeTest(MatxTestFramework):
    ...
    def check_money_supply(self, expected_mat):
        # verify that nodes have the expected MAT supply
        mat_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(mat_supply, [DecimalAmt(expected_mat)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that MAT supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check MAT supply...")
